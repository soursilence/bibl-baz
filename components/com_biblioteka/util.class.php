 ...
 print_r($this->kolumny);*/
  ...
  public function uzupelnijWart($arr,$tb_name){
    if(is_array($arr)&&count($arr)>0){
      foreach($arr as $k => $v){
        if($v==null) unset($this->dane[$tb_name][$k]);
  ...
  public function wyswietlFormularz($id = 0){
   ...
   // echo '<pre>';	print_r($val); echo '</pre>';
    ...
    $wynik .= '<form action="" method="post"><div class="formularz">';
...
//	echo "K: $k, v: ".$val[$k].'<br />';
	...
	if($v['element']=='dict'){
	  ...
	  if($v['label']!="")
					...
					$e.= '<'.$v['element'];
					foreach($v['properties'] as $pk => $pv) $e.=' '.$pk.'="'.$pv.'"';
					if($v['el_end']==true) $e.=' />';
					else  $e.=">".$v['html']; $e.= '</'.$v['element'].'>';
					$wynik .= $e;
					$wynik .= '<br />';
	  }
	}
      ...
      if(count($this->dane['dok_rachbank']['str'])>0)
		foreach($this->dane['dok_rachbank']['str'] as $k => $v){
			$wynik .= '<label for="'.$k.'">'.$v.'</label><input type="text" size="80" name="'.$k.'" id="'.$k.'" value="'.$val[$k].'" />'."<span class=\"err_dis\">&nbsp;".$error_message[$k]."</span><br />\n";

       }
       if(count($this->dane['dok_rachbank']['dat'])>0)
		foreach($this->dane['dok_rachbank']['dat'] as $k => $v){
			if($val[$k]!=null){
				$val[$k] = date('Y-m-d', $val[$k]);
			}
			$wynik .= '<label for="'.$k.'">'.$v.'</label><input type="text" name="'.$k.'" id="'.$k.'" value="'.$val[$k].'" class="jqdata" />'."<span class=\"err_dis\">&nbsp;".$this->error_message[$k]."</span><br />\n";

		}
		if(count($this->dane['dok_rachbank']['num'])>0)
		foreach($this->dane['dok_rachbank']['num'] as $k => $v){
			$wynik .= '<label for="'.$k.'">'.$v.'</label><input type="text" size="80" name="'.$k.'" id="'.$k.'" value="'.$val[$k].'" />'."<span class=\"err_dis\">&nbsp;".$error_message[$k]."</span><br />\n";
...
$wynik .= '$(document).ready(
	   ...
	   $("#numer_archiwizacyjny").attr("value", nr_arch[$(this).attr("value")]);
		...
		$wynik .= '<input type="submit" value="Wyślij" /></div></form>';
		return $wynik;
	}
